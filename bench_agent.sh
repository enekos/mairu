...
output=$(go test ./internal/agent/ -run='^$' -bench='BenchmarkAgentTurn|BenchmarkAgentRunStream|BenchmarkTruncateTailLarge|BenchmarkTruncateHeadLarge|BenchmarkReadFileLineFormatting' -benchtime=100ms -count=3 2>&1)
...
readfile_fmt=$(echo "$output" | grep 'BenchmarkReadFileLineFormatting-' | awk '{sum+=$3; n++} END {if(n>0) printf "%.0f", sum/n}')
...
echo "METRIC readfile_fmt_ns=$readfile_fmt"
